				...
				if (!empty($_GET['page'])) {
					if ($_GET['page'] == "domains") {
						...
						if ($domains = new domains) {
							echo '<h2>Domains</h2>';
							echo '<p>If you delete a domain, all it\'s email accounts and aliases will be deleted too.</p>';
					...
					elseif ($_GET['page'] == "emails") {
						...
						if ($emails = new emails) {
							...
							echo '<p>Here you can add or delete email accounts. You can also reset passwords if your users forget them.</p>';
							echo '<p>Please choose the domain you want to view or modify in the dropdown list.</p>';
							if (!empty($_GET['create'])) {
							...
							if (!empty($_GET['delete'])) {
								if (is_numeric($_GET['delete'])) {
									...
									if ($emails->delete_email($delete)) {
										echo '<div class="success">';
											echo 'The email and all it\'s aliases has been deleted.';
										echo '</div>';
									}
									else {
										echo '<div class="error">';
								...
								if ($domains->get_all_domains()) {
									if ($domains->num_domains() > 0) {
										...
										if (!empty($domainid)) {
											...
											echo '<h3>In database</h3>';
											echo '<p>You can click on an email address to see what aliases has the email as "Destination".<br>To reset a password, click the key icon on the left site of the email you want to change it for and input a new password in the field on the right.</p>';
					...
					?>
					<noscript>Javascript is required for this to work correctly. Please enable javascript or some functions won\'t work!</noscript>
					<?php
				...
				if (!empty($_POST['uname']) && !empty($_POST['pword'])) {
					...
					if (!$member->login($uname, $pword)) {
						...
						if (!$member->echo_login_form()) {
							echo '<div class="error">';
								echo 'An error happened, the login form could not be displayed.';
							echo '</div>';
						}
					}
					else {
						...
						<?php
						echo 'Success! you should now be redirected to the homepage, if not please click <a href="./">here</a>';
					}
				}
				else {
					if (!$member->echo_login_form()) {
